...
void CrosslineDetector::updateCrossline( Mat &frame, vector<bbox_t> detected, vector<outDataCrossline>& output) {
    try 
    {
        Mat in_frame;
        ...
        if (detected.size() != 0) 
        {
            std::vector<Rect2f> subDetect;
            for (auto &i: detected) 
                subDetect.push_back(Rect2f(i.x, i.y, i.w, i.h));

            auto start = getTickCount();

            vector<TrackingTrace> tracks;
            this->tracking->process(subDetect, tracks);
